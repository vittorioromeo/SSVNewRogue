...
{
    NRFactory::NRFactory(
        NRAssets& mAssets, NRGame& mGame, Manager& mManager, World& mWorld)
        : assets(mAssets), game(mGame), manager(mManager), world(mWorld)
    {
    }

    Entity& NRFactory::createTrail(
        const Vec2i& mA, const Vec2i& mB, Color mColor)
    {
    ...
    Entity& NRFactory::createWall(const Vec2i& mPosition)
    {
        ...
        auto& cPhysics(result.createComponent<NRCPhysics>(
            world, true, mPosition, Vec2i{1600, 1600}));
        auto& cRender(
            result.createComponent<NRCRender>(game, cPhysics.getBody()));

        Body& body(cPhysics.getBody());
        body.addGroups(NRGroup::Solid);
        body.addGroupsToCheck(NRGroup::Solid);

        cRender.emplaceSprite(assets.get<Texture>("wall.png"));
    ...
    Entity& NRFactory::createHumanoid(const Vec2i& mPosition)
    {
        ...
        auto& cPhysics(result.createComponent<NRCPhysics>(
            world, false, mPosition, Vec2i{700, 1300}));
        auto& cHumanoid(result.createComponent<NRCHumanoid>(cPhysics));
        auto& cRender(
            result.createComponent<NRCRender>(game, cPhysics.getBody()));
        result.createComponent<NRCHumanoidAnimationController>(
            cRender, cHumanoid);
    ...
    Entity& NRFactory::createPlayer(const Vec2i& mPosition)
    {
        ...
        result.createComponent<NRCPlayer>(
            game, result.getComponent<NRCHumanoid>());

        auto& cRender = result.getComponent<NRCRender>();
        cRender.emplaceSprite(assets.get<Texture>("legs.png"));
        cRender.emplaceSprite(assets.get<Texture>("body.png"));
        cRender.emplaceSprite(assets.get<Texture>("arms.png"));
        cRender.emplaceSprite(assets.get<Texture>("head.png"));
    ...
    {
        auto& result(createHumanoid(mPosition));
        result.createComponent<NRCWanderer>(
            game, result.getComponent<NRCHumanoid>());

        auto& cRender = result.getComponent<NRCRender>();
        cRender.emplaceSprite(assets.get<Texture>("legs.png"));
        cRender.emplaceSprite(assets.get<Texture>("body2.png"));
        cRender.emplaceSprite(assets.get<Texture>("arms.png"));
        cRender.emplaceSprite(assets.get<Texture>("head.png"));
