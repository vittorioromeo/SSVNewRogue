...
NRGame::NRGame(GameWindow& mGameWindow, NRAssets& mAssets)
    : gameWindow(mGameWindow), assets(mAssets),
      ...
      grid(world.getSpatial()), debugText{assets.get<BitmapFont>("limeStroked")}
{
...
void NRGame::initInput()
{
    ...
    [this](FT)
    {
        ...
        // debugGrid[index.x + grid.getOffset()][index.y + grid.getOffset()] =
        // 1;
    },
    t::Once);
...
#define NRGAME_TEST_BENCH
...
#ifdef NRGAME_TEST_BENCH
                if(getRndI(0, 100) > 90)
                    factory.createWanderer({1600 * iX + 800, 1600 * iY + 800});
    ...
    playerCHumanoid.additionalSpeed.addModifier(b); });
        ...
        buff.append<Do>([&]{ playerCHumanoid.additionalSpeed.removeModifier(b);
...
void NRGame::drawDebugGrid()
{
    ...
    for(int iX{0}; iX < grid.getColumns(); ++iX)
        for(int iY{0}; iY < grid.getRows(); ++iY) {
            ...
            Vec2i c{
            grid.getCellSize() * (oIX + 1), grid.getCellSize() * (oIY + 1)};
