...
namespace nr
{
    NRGame::NRGame(GameWindow& mGameWindow, NRAssets& mAssets)
        : gameWindow(mGameWindow), assets(mAssets),
          factory{assets, *this, manager, world}, world(1000, 1000, 1000, 500),
          grid(world.getSpatial()),
          debugText{assets.get<BitmapFont>("limeStroked")}
    {
    ...
    void NRGame::initInput()
    {
        ...
        gameState.addInput({{k::Escape}}, [this](FT)
            {
                gameWindow.stop();
            });

        gameState.addInput({{k::A}}, [=](FT)
            {
                camera.pan(-10, 0);
            });
        gameState.addInput({{k::D}}, [=](FT)
            {
                camera.pan(10, 0);
            });
        gameState.addInput({{k::W}}, [=](FT)
            {
                camera.pan(0, -10);
            });
        gameState.addInput({{k::S}}, [=](FT)
            {
                camera.pan(0, 10);
            });
        gameState.addInput({{k::Q}}, [=](FT)
            {
                camera.zoomOut(1.1f);
            });
        gameState.addInput({{k::E}}, [=](FT)
            {
                camera.zoomIn(1.1f);
            });
        ...
        gameState.addInput({{k::Num1}},
            [this](FT)
            {
                factory.createWall(getMousePosition());
            },
            t::Once);
        gameState.addInput({{k::Num2}}, [this](FT)
            {
                factory.createWanderer(getMousePosition());
            });
        gameState.addInput({{k::Num3}},
            [this](FT)
            {
                factory.createPlayer(getMousePosition());
            },
            t::Once);
        gameState.addInput({{k::Num4}},
            [this](FT)
            {
                auto index(grid.getIdx(getMousePosition()));
                auto count(grid.getCell(index.x, index.y).getBodies().size());
                lo() << index.x << " " << index.y << "  :: " << count << "\n";
                // debugGrid[index.x + grid.getOffset()][index.y +
                // grid.getOffset()] =
                // 1;
            },
            t::Once);
        gameState.addInput({{k::Num5}},
            [this](FT)
            {
                clearDebugGrid();
            },
        ...
        gameState.addInput({{k::Num6}}, [this](FT)
            {
                auto body(world.getQuery<QueryType::Point>(getMousePosition())
                              .next());
...
#define NRGAME_TEST_BENCH
...
#ifdef NRGAME_TEST_BENCH
                    if(getRndI(0, 100) > 90)
                        factory.createWanderer(
                            {1600 * iX + 800, 1600 * iY + 800});
                    else if(getRndI(0, 100) > 90)
                        factory.createWall({1600 * iX + 800, 1600 * iY + 800});
#endif
                }
        ...
        factory.createWall(
            {1600 * 7, 1600 * 13 + 800 - 1300 - 1300 - 1 - 1600});
        ...
        /*{
            ...
            buff.append<Do>([&]{ b.onCompute += [&](ssvrpg::Value<int>&, int&
        mCurrent){ mCurrent += 200; };
        playerCHumanoid.additionalSpeed.addModifier(b); });
            buff.append<Wait>(100);
            buff.append<Do>([&]{
        playerCHumanoid.additionalSpeed.removeModifier(b);
        playerCHumanoid.modifierManager.del(b);
    ...
    void NRGame::render(const Drawable& mDrawable)
    {
    ...
    void NRGame::drawDebugGrid()
    {
        ...
        for(int iX{0}; iX < grid.getColumns(); ++iX)
            for(int iY{0}; iY < grid.getRows(); ++iY)
            {
                ...
                Vec2i b{
                    grid.getCellSize() * (oIX + 1), grid.getCellSize() * oIY};
                Vec2i c{grid.getCellSize() * (oIX + 1),
                    grid.getCellSize() * (oIY + 1)};
                Vec2i d{
                    grid.getCellSize() * oIX, grid.getCellSize() * (oIY + 1)};
