...
namespace nr
{
    ...
    NRCHumanoid::NRCHumanoid(sses::Entity& mE, NRCPhysics& mCPhysics)
        : sses::Component{mE}, cPhysics(mCPhysics),
          unCrouchSensor{cPhysics, Vec2i{700, 1300}},
          autoCrouchTopSensor{cPhysics, Vec2i(100, 100)},
          autoCrouchBottomSensor{cPhysics, Vec2i(100, 100)},
          body(cPhysics.getBody()), standingHeight{body.getHeight()}
    {
        ...
        body.onPostUpdate += [&]
        {
            unCrouchSensor.setPosition(
                {s.getX(), s.getBottom() - ucsShape.getHalfHeight()});

            int atcsX{s.getLeft() - actsShape.getHalfWidth()};
            int abcsX{s.getLeft() - acbsShape.getHalfWidth()};
            int atcsY{
                s.getBottom() - standingHeight + actsShape.getHalfHeight()};
            int abcsY{
                s.getBottom() - crouchingHeight + acbsShape.getHalfHeight()};
            if(!facingLeft)
            {
        ...
        cPhysics.onResolution += [&](const Vec2i& mMinIntersection)
        {
    ...
    void NRCHumanoid::update(FT)
    {
        ...
        if(!isInAir() && autoCrouchTopSensor.isActive() &&
            !autoCrouchBottomSensor.isActive())
        {
            autoCrouching = true;
            crouch(true);
        }
        ...
        if(!isInAir())
        {
            if(!crouching)
            {
                ...
                else if(abs(velocity.x) >= runSpeed)
                    action = Action::Running;
                else if(abs(velocity.x) >= walkSpeed)
                    action = Action::Walking;
            }
            else
                action =
                    velocity.x == 0 ? Action::Crouching : Action::CrouchWalking;
        }
        else
        {
            if(velocity.y > 0)
    ...
    void NRCHumanoid::unCrouch()
    {
        ...
        if(crouching)
        {
            body.setPosition(body.getPosition() -
                             Vec2i{0, (standingHeight - crouchingHeight) / 2});
